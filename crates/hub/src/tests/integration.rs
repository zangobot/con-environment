...
async fn test_orchestrator_creates_pod_and_service() {
    ...
    let binding = orchestrator::get_or_create_pod(user_id, ctx.config.clone()).await;
    ...
    assert!(
        ctx.service_exists(&binding.service_name).await,
        "Service should exist"
    );

    // Verify we can retrieve the same binding (idempotency)
    let second_binding = orchestrator::get_or_create_pod(user_id, ctx.config.clone()).await;

    assert!(second_binding.is_ok(), "Should retrieve existing pod");
    let second_binding = second_binding.unwrap();

    assert_eq!(
        binding.pod_name, second_binding.pod_name,
        "Should return same pod"
    );
    assert_eq!(
        binding.service_name, second_binding.service_name,
...
async fn test_pod_limit_enforcement() {
    ...
    let user1 = orchestrator::get_or_create_pod("limit-user-1", limited_config.clone()).await;
    assert!(user1.is_ok(), "First pod should succeed");

    let user2 = orchestrator::get_or_create_pod("limit-user-2", limited_config.clone()).await;
    assert!(user2.is_ok(), "Second pod should succeed");

    // This should fail due to limit
    let user3 = orchestrator::get_or_create_pod("limit-user-3", limited_config.clone()).await;
...
async fn test_concurrent_pod_creation() {
    ...
    for i in 0..5 {
        ...
        let handle =
            tokio::spawn(async move { orchestrator::get_or_create_pod(&user_id, config).await });
